...
double ABSUtilities::DiscountMargin(List<DateTime>^ Dte, List<double>^ Flws, ManBloombergVector^ BaseRate,int Daycount, double Guess,int precision){
	if(Guess<=0.0 || Guess>1.0) Guess=0.05;
	unsigned int CurrentIterations=0;
	double Result=Guess;
	double PreviuousGuess=Guess+0.01;
	double CurrentGuess=Guess;
