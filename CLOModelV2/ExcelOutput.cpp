...
HRESULT ExcelOutput::PrintWaterfallStep(const QString& DestinationSheet, const QString& DestinationAddress, const QString& StepStructure) {
	ExcelCommons::InitExcelOLE();
	HRESULT hr;
	static DISPID dispid = 0;
	DISPPARAMS Params;
	...
	try {
		Command[CurrentCmdIndex].vt = VT_BSTR;
		Command[CurrentCmdIndex--].bstrVal = SysAllocString(L"PrintWaterfallStep");
		Command[CurrentCmdIndex].vt = VT_BSTR;
		Command[CurrentCmdIndex--].bstrVal = SysAllocString(DestinationSheet.toStdWString().c_str());
		Command[CurrentCmdIndex].vt = VT_BSTR;
		Command[CurrentCmdIndex--].bstrVal = SysAllocString(DestinationAddress.toStdWString().c_str());
		Command[CurrentCmdIndex].vt = VT_BSTR;
		Command[CurrentCmdIndex--].bstrVal = SysAllocString(StepStructure.toStdWString().c_str());
		Params.rgdispidNamedArgs = NULL;
		Params.rgvarg = Command;
		Params.cArgs = 4;
		Params.cNamedArgs = 0;
