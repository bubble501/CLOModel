...
public:
	...
	virtual void SetStock(qint32 FlowTpe);
	virtual double GetStock(const QDate& index, qint32 FlowTpe)const;
	virtual double GetStock(int index, qint32 FlowTpe)const;
protected:
	static bool SamePeriod(const QDate& a, const QDate& b, CashFlowAggregation Freq);
	QMap<QDate, QHash<qint32, double>* > m_CashFlows;
	QHash<qint32, QMap<QDate, double>* > m_Stocks;
