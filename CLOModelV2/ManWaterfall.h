...
namespace ManagedCLO {
	...
	public:
		...
		*/
		...
		/*!
		\brief Returns the specified reserve fund in the waterfall
		\arg RFindex the index of the reserve fund to retrieve. If out of range null is returned
		\details Returns the RFindex-th reserve fund from the model.
		*/
		ManReserveFund^ GetReserveFund(int RFindex) { 
			const ReserveFund* const TempRes = Unmanaged->GetReserveFund(RFindex);
			if (TempRes)
				return gcnew ManReserveFund(*(TempRes));
			return nullptr;
		}
		/*!
		...
		*/
		void SetReserveFund(int RFindex, String^ RFtarget, String^ RFmultiple, String^ RFfloor, String^ RFcap, double RFcurrent, int RFfreed, bool RFtoInterest) { Unmanaged->SetReserveFund(RFindex, String2QString(RFtarget), String2QString(RFmultiple), String2QString(RFfloor), String2QString(RFcap), RFcurrent, RFfreed, RFtoInterest); }
		/*!
		\brief Set up a reserve fund
		\arg source The reserve fund to copy
		\details This function will set up the specified reserve fund as a copy of the supplied one
		*/
		void SetReserveFund(int RFindex, ManReserveFund^ source) { Unmanaged->SetReserveFund(RFindex, *(source->Unmanaged)); }
		/*!
		\brief Add a reserve fund to the model
		\arg RFtarget a Bloomberg vector representing the target level of the reserve fund.
		\arg RFmultiple a Bloomberg vector representing the value by which RFtarget will be multiplied.
		\arg RFfloor a Bloomberg vector representing the floor level of the reserve fund. For non-amortizing RFs set to "0"
		\arg RFcurrent the current size of the reserve fund
		\arg RFfreed A seniority level. When all the notes not junior to this seniority level will be redeemed, the RF will be released as excess spread. 0 for freeing it only at maturity
		\arg RFtoInterest Set to true if the reserve fund will be part of interest proceeds when calculating the waterfall, set to false if it's part of principal proceeds.
		\details This function will add a reserve fund to the model according to the supplied parameters.<br/>If RFtarget is a seniority level, the reserve fund target amount will be the sum of all the notes that rank not junior to that level, otherwise the reserve will be a non amortizing.<br/>The target level will then be multiplied by RFmultiple to assess the actual replenishment target.<br/>In any case, the target won't be set below the RFfloor amount.
		*/
		void AddReserveFund(String^ RFtarget, String^ RFmultiple, String^ RFfloor, String^ RFcap, double RFcurrent, int RFfreed, bool RFtoInterest) { Unmanaged->AddReserveFund(String2QString(RFtarget), String2QString(RFmultiple), String2QString(RFfloor), String2QString(RFcap), RFcurrent, RFfreed, RFtoInterest); }
		/*!
		\brief Add a reserve fund
		\arg source The reserve fund to add
		\details This function will add the supplied reserve fund to the model
		*/
		void AddReserveFund( ManReserveFund^ source) { Unmanaged->AddReserveFund(*(source->Unmanaged)); }
		/*!
		\brief Removes all the settings for the specified reserve fund
		\arg index the index of the reserve fund. -1 will reset all the reserves. If out of range the method will do nothing
		...
		*/
		...
		/*!
		\brief Removes all the the specified reserve fund from the model
		\arg index the index of the reserve fund. -1 will remove all the reserves. If out of range the method will do nothing
		\details This function will remove the specified reserve fund from the waterfall.
		*/
		void RemoveReserve(int index) { Unmanaged->RemoveReserve(index); }
		/*!
		\brief The name of the deal
		*/
		property String^ DealName{
			String^ get() { return QString2String(Unmanaged->GetDealName()); }
			void set(String^ a) { Unmanaged->SetDealName(String2QString(a)); }
		}
		/*!
		\brief Share AUM paid as senior expenses
		\details Bloomberg vector representing the share of AUM to be paid as senior expenses
		*/
		property String^ SeniorExpenses{
			String^ get() { return QString2String(Unmanaged->GetSeniorExpenses()); }
			void set(String^ a) { Unmanaged->SetSeniorExpenses(String2QString(a)); }
		}
		/*!
		\brief Share AUM paid as senior fees
		\details Bloomberg vector representing the share of AUM to be paid as senior fees
		*/
		property String^ SeniorFees{
			String^ get() { return QString2String(Unmanaged->GetSeniorFees()); }
			void set(String^ a) { Unmanaged->SetSeniorFees(String2QString(a)); }
		}
		/*!
		\brief Share AUM paid as junior fees
		\details Bloomberg vector representing the share of AUM to be paid as junior fees
		*/
		property String^ JuniorFees{
			String^ get() { return QString2String(Unmanaged->GetJuniorFees()); }
			void set(String^ a) { Unmanaged->SetJuniorFees(String2QString(a)); }
		}
		/*!
		\brief Fixed amount paid as senior expenses
		\details Bloomberg vector representing the fixed amount paid as senior expenses
		*/
		property String^ SeniorExpensesFixed{
			String^ get() { return QString2String(Unmanaged->GetSeniorExpensesFixed()); }
			void set(String^ a) { Unmanaged->SetSeniorExpensesFixed(String2QString(a)); }
		}
		/*!
		\brief Fixed amount paid as senior fees
		\details Bloomberg vector representing the fixed amount paid as senior fees
		*/
		property String^ SeniorFeesFixed{
			String^ get() { return QString2String(Unmanaged->GetSeniorFeesFixed()); }
			void set(String^ a) { Unmanaged->SetSeniorFeesFixed(String2QString(a)); }
		}
		/*!
		\brief Fixed amount paid as junior fees
		\details Bloomberg vector representing the fixed amount paid as junior fees
		*/
		property String^ JuniorFeesFixed{
			String^ get() { return QString2String(Unmanaged->GetJuniorFeesFixed()); }
			void set(String^ a) { Unmanaged->SetJuniorFeesFixed(String2QString(a)); }
		}
		/*!
