...
namespace ManagedCLO {
	 ...
	 */
	public ref class ManBoolVector
	{
	private:
		static const unsigned int ClassIdentity=9;
	internal:
		BoolVector* Unmanaged;
		ManBoolVector(const BoolVector& a){Unmanaged=new BoolVector(a);}
	public:			
		...
		*/
		...
		ManBoolVector(){Unmanaged=new BoolVector();}
		//! Copy constructor
		ManBoolVector(const ManBoolVector% a){Unmanaged=new BoolVector(*(a.Unmanaged));}

		//! Copy constructor for ref class
		ManBoolVector(ManBoolVector^ a){Unmanaged=new BoolVector(*(a->Unmanaged));}

		/*! 
		\brief Constructor taking the vector string
		\details If the supplied vector is not a valid boolean vector the class will default to an empty vector
		*/
		ManBoolVector(String^ Vec){Unmanaged=new BoolVector(String2QString(Vec));}
		/*! 
		\brief Constructor taking the vector string and the anchor date
		\details If the supplied vector is not a valid boolean vector the class will default to an empty vector.<br/>If the vector string starts with an anchor date, it will be overwritten by the supplied one.
		*/
		ManBoolVector(String^ Vec,DateTime Anchor){Unmanaged=new BoolVector(String2QString(Vec),DateTime2QDate(Anchor));}
		/*! 
		\brief Gets the vector value for the specified date
		\arg index the date for which to get the value of the vector
		\returns 'Y'(89) if the vector at the supplied index is true, 'N'(78) if it's false and 0 if the vector or the index are invalid
		...
		*/
		...
		\details This function will try to set the vector to represent the supplied string, if it's not a valid boolean vector string the vector won't be changed and false is returned
		*/
