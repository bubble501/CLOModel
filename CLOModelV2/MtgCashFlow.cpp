...
MtgCashFlow MtgCashFlow::ApplyScenario(BloombergVector CPRv, BloombergVector CDRv, BloombergVector LSv) const {
	...
	for (i = 1; i < Count(); i++) {
		...
		Result.AddFlow(GetDate(i), (ApplicablePrincipal - SumDeltaOut)*CPRv.GetSMM(GetDate(i), 1), PrepaymentFlow);
		SumDeltaOut += (ApplicablePrincipal - SumDeltaOut)*CPRv.GetSMM(GetDate(i), 1);
		Result.AddFlow(GetDate(i), Result.GetAccruedInterest(GetDate(i))*CPRv.GetSMM(GetDate(i), 1), InterestFlow);
		Result.AddFlow(GetDate(i), -Result.GetAccruedInterest(GetDate(i))*CPRv.GetSMM(GetDate(i), 1), AccruedInterestFlow);



		TempF = (ApplicablePrincipal - SumDeltaOut)*CDRv.GetSMM(GetDate(i), 1);
		Result.AddFlow(GetDate(i), TempF, PrincipalDefault);
		Result.AddFlow(GetDate(i), qMin(TempF, TempF*LSv.GetValue(GetDate(i))), LossFlow);
		Result.AddFlow(GetDate(i), qMin(TempF, TempF*(1.0 - LSv.GetValue(GetDate(i)))), PrincipalRecovered);
		SumDeltaOut += TempF;
		SumDeltaOut -= qMin(TempF, TempF*(1.0 - LSv.GetValue(GetDate(i))));

		TempF = Result.GetAccruedInterest(GetDate(i))*CDRv.GetSMM(GetDate(i), 1);
		if (TempF >= 0.01) {
			Result.AddFlow(GetDate(i), qMin(TempF, -TempF*LSv.GetValue(GetDate(i))), AccruedInterestFlow);
			Result.AddFlow(GetDate(i), qMin(TempF, TempF*(1.0 - (LSv.GetValue(GetDate(i))))), InterestRecovered);
			Result.AddFlow(GetDate(i), qMin(TempF, TempF*(LSv.GetValue(GetDate(i)))), LossOnInterestFlow);
		}
		Result.AddFlow(GetDate(i), ApplicablePrincipal - SumDeltaOut, AmountOutstandingFlow);
		if (GetFlow(i,AmountOutstandingFlow) == 0.0)
			Result.AddFlow(GetDate(i), 0.0, WACouponFlow);
		else
