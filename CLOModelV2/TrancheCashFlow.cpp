...
double TrancheCashFlow::GetPreviousDeferred(const QDate& CurrentDate){
	if(Deferred.isEmpty()) return StartingDeferredInterest;
...
QDate TrancheCashFlow::GetLastFlowDate(bool IncludeDeferred) const {
	int TempIndex = GetLastFlowIndex(IncludeDeferred);
	if (TempIndex >= 0) return FlowDates.at(TempIndex);
	return QDate();
}
int TrancheCashFlow::GetLastFlowIndex(bool IncludeDeferred) const {
	for (int i = Count() - 1; i >= 0; i--) {
		if (
			Interest.at(i) > 0.009
			|| Principal.at(i) > 0.009
			|| (IncludeDeferred && Deferred.at(i) > 0.009)
			) return i;
	}
	return -1;
}