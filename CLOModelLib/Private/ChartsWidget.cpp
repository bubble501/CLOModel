	...
	if (a.GetNumReserves() >0){//Reserve Funds
		...
		for(int ReserveIter=0;ReserveIter<a.GetNumReserves();ReserveIter++){
			...
			for(int i=0;i<CurrentResFlows.Count();i++){
				QModelIndex TempIndex=ChartsModels.last()->index(i,0);
				ChartsModels.last()->setData(TempIndex, (CurrentResFlows.GetFlow(i, ReserveFund::ReserveFlowsType::ReplenishFromInterest) + CurrentResFlows.GetFlow(i, ReserveFund::ReserveFlowsType::ReplenishFromPrincipal)) / 1000000.0);
				ChartsModels.last()->setData(TempIndex, "Reserve Level - Date: " + CurrentResFlows.GetDate(i).toString("MMM-yy") + " - Value: " + Commarize((CurrentResFlows.GetFlow(i, ReserveFund::ReserveFlowsType::ReplenishFromInterest) + CurrentResFlows.GetFlow(i, ReserveFund::ReserveFlowsType::ReplenishFromPrincipal))), Qt::ToolTipRole);
				TempIndex=ChartsModels.last()->index(i,1);
				ChartsModels.last()->setData(TempIndex, CurrentResFlows.GetFlow(i, ReserveFund::ReserveFlowsType::ShortFall) / 1000000.0);
				ChartsModels.last()->setData(TempIndex, "Reserve Target - Date: " + CurrentResFlows.GetDate(i).toString("MMM-yy") + " - Value: " + Commarize(CurrentResFlows.GetFlow(i, ReserveFund::ReserveFlowsType::ShortFall)), Qt::ToolTipRole);
